	...
	this.filterData = function(e){
		...
		function markREST(node, path) {
			if (path.length === level) {
                ...
                if(level > 0){
                	parentPath = level - 1;
                	var restDim = RowColCheck == 'row' ? self.rows : self.cols;
					
                	orderingCol = SQLike.q({
						...
						'Where': function() {
							var checkVal = false;
							topBottomWhere = this
							$.each(path,function(_i,_d){
								if(topBottomWhere[restDim[_i].name] == _d){
									checkVal=true;
                                }else{
                                    checkVal=false;
                                } 
...
//					if(topBottomInfo.TOPBOTTOM_TYPE == 'Top'){
					if (i < TopBottomCount){
						relElements.push(reOrdered[i].value);
                        continue;
					}
//					}else{
...
//				var r    = [...restElements].sort();
				...
				/*총계기준으로 topbottom 설정*/
				dataSource = dataSource.sort(function compare(a, b) {
				   
				  var aValue = relElements.indexOf(a[topBottomInfo.APPLY_FLD_NM])
				  var bValue = relElements.indexOf(b[topBottomInfo.APPLY_FLD_NM])
...
//			this.csvData = SQLikeUtil.doSqlLike(this.dataSourceId, csvDataConfig);
			...
			var renderPivot = function(self, data){
				/* DOGFOOT syjin 측정값이 null 값일 경우 변동 측정값이 안나오던 현상 수정 20220111*/
				//dev에서 변동 측정값 계산 할 때, 측정값이 wise_null_value 일 경우 string 형식이므로 null 값 반환 
				if(self.deltaItems.length > 0){
					var dataMeasures = [];

					$.each(self.meta.DataItems.Measure, function(_i, _v){
						dataMeasures.push(_v.UNI_NM);
					})
                    
                    $.each(dataMeasures, function(_i, _v){
                    	$.each(data,function(_dataIndex,_dataVal){
                    		if(_dataVal[_v] != undefined){
								if(_dataVal[_v] == "wise_null_value"){
									_dataVal[_v] = 0;
								}
                    		}
					    });
